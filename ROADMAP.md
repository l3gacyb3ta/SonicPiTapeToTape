...
|---------|--------------|------------|
| **v1.5.0-beta.4** | `latest`, `beta` | **Rebrand to Sonic Web** — `@mjayb/sonicweb`, repo `SonicWeb`, domain `sonicweb.cc` (#610/#611/#614). Engine **self-sufficient by default** (#604). `Hash#length`/`#size`/`#count` → `__spSize` (#603). `spread` Bjorklund parity with desktop `redistribute` (#597/#598). Dynamic `use_synth`, inline-loop RNG, cross-loop `get`/`set`, bang-only mixer/volume lint parity fixes (#585–#596). Playable docs + parity dashboards. CI checkout/setup-node v6/v7 (#607). Install: `npm install @mjayb/sonicweb`. |
